...
function firebaseMapper(doc: DocumentData): RendaProps {
...
export async function createRenda(
  props: Omit<RendaProps, "id" | "created_at">
) {
  const inputData = {
    tipo: props.tipo,
...
export async function createBulkRendas(
  inputs: Omit<RendaProps, "id" | "created_at">[]
) {
...
export async function listarTodasRendas(
  filtros?: Filtros
): Promise<RendaProps[]> {
