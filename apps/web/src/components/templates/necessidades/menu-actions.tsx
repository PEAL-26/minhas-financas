...
import { removeNecessidade } from '@/services/necessidades';
...
export function MenuActions({ id }: { id: string }) {
  ...
  const handleOpenEdit = () =>
    showModalRegisto('Editar necessidade', <FormularioRegistoNecessidade id={id} />);

  const handleOpenRemove = () =>
    showModalRemove({
      id,
      removeFn: removeNecessidade,
      queryKey: ['necessidades'],
    });

  const handleGoToDetalhes = () => route.push(`/necessidades/${id}`);
