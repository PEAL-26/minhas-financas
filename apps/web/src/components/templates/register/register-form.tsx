...
export function RegisterForm() {
  const { form, handleSubmit, isLocking, isSubmitting, setIsLocking } = useRegisterForm();

  return (
    <Form {...form}>
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-4 p-6">
          <InputFormControl control={form.control} type="text" label="Nome" name="name" />
          <InputFormControl control={form.control} type="email" label="Email" name="email" />
          <InputFormControl control={form.control} type="password" label="Senha" name="password" />
        </div>
        ...
        <div className="p-6 pt-0">
          <Button
            ...
            className="w-full"
          >
            {isSubmitting && <Loader2Icon size={28} className="animate-spin" />}Cadastra-se
          </Button>
